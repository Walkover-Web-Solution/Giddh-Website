...
const FeatureItem = memo(({ feature, index, isExpanded, onToggle }) => {
  ...
  return (
    <div
      className={`d-flex flex-column gap-1 rounded p-2 ${isExpanded ? "bg-accent text-white border" : ""
        } cursor-pointer`}
    >
      <div className="d-flex align-items-center gap-2" onClick={handleToggle}>
        ...
        <h3
          className={`col-primary c-fw-600 c-fs-4 m-0 ${isExpanded ? "text-white" : ""
            }`}
        >
          {feature?.name}
        </h3>
      ...
      <div
        className={`d-flex flex-column gap-2 pt-2 ps-sm-5 ${isExpanded ? "d-flex" : "d-none"
          }`}
      >
