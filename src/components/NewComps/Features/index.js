  ...
  return (
    <section className="container section_py d-flex flex-column gap-5">
      {data?.globalCardData?.map((feature, index) => {
        return (
          <div className={`d-flex gap-5 flex-column flex-md-row align-items-center ${index % 2 === 1 ? 'flex-md-row-reverse' : ''}`}>
            <div>
            ...
            <div className={style.img_cont}>
              ...
              <Image src={`/img/feature-gallery-icons${feature?.image}`} className={style.img} width={440} height={440} alt={feature?.name}/>
            </div>
          </div>
        );
      })}
