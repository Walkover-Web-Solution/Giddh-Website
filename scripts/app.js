...
function runBlock($rootScope, $window, $http, localStorageService, $anchorScroll) { // $q,
    ...
    function GetElectronAppVersion() {
        ...
        $http.get('https://s3-ap-southeast-1.amazonaws.com/tetingmankuuuuu/latest.yml', {
            ...
            var version = res.data.split('files')[0];
            var versNum = version.split(' ')[1];
            $rootScope.apkVersion = versNum.split(' ')[0].replace(/\s/g, "");;
        }, function(error) {
            console.log(error);
        });
    }
...
angular.module('razor-pay', []).directive('razorPay', [
    '$compile', '$filter', '$document', '$parse', '$rootScope', '$timeout', 'toastr',
    function($compile, $filter, $document, $parse, $rootScope, $timeout, toastr) {
        return {
            restrict: 'A',
            scope: false,
            transclude: false,
